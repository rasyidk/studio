...
export default function Home() {
  ...
  const handleClassifyOutcomeCategory = async () => {
    if (!pdfData?.pages || pdfData.pages.length === 0) {
        toast({ variant: "destructive", title: "Error", description: "No PDF text available to classify." });
        return;
    }
    setIsClassifyingOutcomeCategory(true);
    setOutcomeCategory(null);
    try {
        const pdfText = pdfData.pages.join('\n\n');
        const result = await classifyOutcomeCategory({ pdfText });
        setOutcomeCategory(result);
    } catch (error) {
        console.error("AI classification failed", error);
        toast({ variant: "destructive", title: "AI Error", description: "Failed to classify outcome category. Please try again." });
    } finally {
        setIsClassifyingOutcomeCategory(false);
    }
  }
  ...
  const handleClassifyMeasures = async () => {
    if (!pdfData?.pages || pdfData.pages.length === 0) {
        toast({ variant: "destructive", title: "Error", description: "No PDF text available to classify." });
        return;
    }
    setIsClassifyingMeasures(true);
    setMeasures(null);
    try {
        const pdfText = pdfData.pages.join('\n\n');
        const result = await classifyMeasures({ pdfText });
        setMeasures(result);
    } catch (error) {
        console.error("AI classification failed", error);
        toast({ variant: "destructive", title: "AI Error", description: "Failed to classify measures. Please try again." });
    } finally {
        setIsClassifyingMeasures(false);
    }
  }
  ...
  const handleClassifyAiDisclosureRequired = async () => {
    if (!pdfData?.pages || pdfData.pages.length === 0) {
        toast({ variant: "destructive", title: "Error", description: "No PDF text available to classify." });
        return;
    }
    setIsClassifyingAiDisclosureRequired(true);
    setAiDisclosureRequired(null);
    try {
        const pdfText = pdfData.pages.join('\n\n');
        const result = await classifyAiDisclosureRequired({ pdfText });
        setAiDisclosureRequired(result);
    } catch (error) {
        console.error("AI classification failed", error);
        toast({ variant: "destructive", title: "AI Error", description: "Failed to classify AI disclosure requirement. Please try again." });
    } finally {
        setIsClassifyingAiDisclosureRequired(false);
    }
  }
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:items-start">
            ...
            <div className="flex flex-col gap-8">
              ...
              <Card>
                <CardHeader>
                    <CardTitle className="font-headline flex items-center justify-between text-2xl">
                      <div className="flex items-center gap-2"><Target className="text-primary"/> Outcome Category</div>
                      <Button size="sm" onClick={handleClassifyOutcomeCategory} disabled={anyLoading}>
                        {isClassifyingOutcomeCategory ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
                        Classify
                      </Button>
                    </CardTitle>
                    <CardDescription>Identify the outcome category(ies) reported in the paper.</CardDescription>
                </CardHeader>
                { (isClassifyingOutcomeCategory || outcomeCategory) &&
                <CardContent>
                    {isClassifyingOutcomeCategory ? (
                        <div className="flex items-center justify-center">
                            <Loader2 className="h-8 w-8 animate-spin text-primary" />
                        </div>
                    ) : outcomeCategory ? (
                        <div className="space-y-4">
                            <div className="flex justify-center">
                                <Badge variant="secondary" className="text-lg">{outcomeCategory.outcomeCategory}</Badge>
                            </div>

                            {outcomeCategory.sources && outcomeCategory.sources.length > 0 && (
                                <>
                                    <Separator/>
                                    <div className="space-y-4 pt-4">
              ...
              <Card>
                <CardHeader>
                    <CardTitle className="font-headline flex items-center justify-between text-2xl">
                      <div className="flex items-center gap-2"><Ruler className="text-primary"/> Measures</div>
                      <Button size="sm" onClick={handleClassifyMeasures} disabled={anyLoading}>
                        {isClassifyingMeasures ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
                        Classify
                      </Button>
                    </CardTitle>
                    <CardDescription>Identify the measures or instruments used in the paper.</CardDescription>
                </CardHeader>
                { (isClassifyingMeasures || measures) &&
                <CardContent>
                    {isClassifyingMeasures ? (
                        <div className="flex items-center justify-center">
                            <Loader2 className="h-8 w-8 animate-spin text-primary" />
                        </div>
                    ) : measures ? (
                        <div className="space-y-4">
                            <div className="flex justify-center">
                                <Badge variant="secondary" className="text-lg">{measures.measures}</Badge>
                            </div>

                            {measures.sources && measures.sources.length > 0 && (
                                <>
                                    <Separator/>
                                    <div className="space-y-4 pt-4">
              ...
              <Card>
                <CardHeader>
                    <CardTitle className="font-headline flex items-center justify-between text-2xl">
                      <div className="flex items-center gap-2"><FileKey className="text-primary"/> AI Disclosure Required</div>
                      <Button size="sm" onClick={handleClassifyAiDisclosureRequired} disabled={anyLoading}>
                        {isClassifyingAiDisclosureRequired ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
                        Classify
                      </Button>
                    </CardTitle>
                    <CardDescription>Identify whether the paper requires AI disclosure.</CardDescription>
                </CardHeader>
                { (isClassifyingAiDisclosureRequired || aiDisclosureRequired) &&
                <CardContent>
                    {isClassifyingAiDisclosureRequired ? (
                        <div className="flex items-center justify-center">
                            <Loader2 className="h-8 w-8 animate-spin text-primary" />
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-center">
                                <Badge variant="secondary" className="text-lg">{aiDisclosureRequired.aiDisclosureRequired}</Badge>
                            </div>

                            {aiDisclosureRequired.sources && aiDisclosureRequired.sources.length > 0 && (
                                <>
                                    <Separator/>
                                    <div className="space-y-4 pt-4">
